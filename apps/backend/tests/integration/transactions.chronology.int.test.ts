import { createTransaction, registerAndCreateWallet, resetDatabase } from '../helpers/testUtils';
...
describe('Transactions chronology', () => {
    ...
    it('returns transactions in deterministic order: createdAt desc, id desc', async () => {
        ...
        const listResponse = await agent.get(`/api/wallets/${walletId}/transactions`);
        ...
        const expectedSecondAndThird = [txA.body.id, txB.body.id].sort((a: string, b: string) => b.localeCompare(a));
    ...
    it('rejects update that breaks chronological balance', async () => {
        ...
        const invalidUpdate = await agent.patch(`/api/wallets/${walletId}/transactions/${buyTx.body.id}`).send({
            quantity: 0.4,
        });

        expect(invalidUpdate.status).toBe(400);
        expect((invalidUpdate.body.error || '').toLowerCase()).toContain('chronological');
    });
    ...
    it('does not allow access to another user wallet', async () => {
        ...
        const listResponse = await stranger.agent.get(`/api/wallets/${owner.walletId}/transactions`);
        expect(listResponse.status).toBe(404);

        const createResponse = await stranger.agent.post(`/api/wallets/${owner.walletId}/transactions`).send({
            buyOrSell: 'buy',
            coinSymbol: 'btc',
            price: 100,
            quantity: 1,
