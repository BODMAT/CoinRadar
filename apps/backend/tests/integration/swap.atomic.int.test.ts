...
describe("Swap atomic/concurrency behavior", () => {
  ...
  it("creates two linked transactions on successful swap", async () => {
    ...
    const swapResponse = await agent
      .post(`/api/wallets/${walletId}/swap`)
      .send({
        fromCoin: "btc",
        fromQuantity: 1,
        fromPrice: 120,
    ...
    expect(swapResponse.body.sell.swapGroupId).toBe(
      swapResponse.body.swapGroupId,
    );
    expect(swapResponse.body.buy.swapGroupId).toBe(
      swapResponse.body.swapGroupId,
    );
  });
  ...
  it("rejects swap with insufficient funds and does not create swap transactions", async () => {
    ...
    const beforeCount = await prisma.transaction.count({ where: { walletId } });
    ...
    const afterCount = await prisma.transaction.count({ where: { walletId } });
  ...
  it("keeps consistent balance under concurrent swaps", async () => {
    ...
    expect(statuses.some((status) => status === 400 || status === 409)).toBe(
      true,
    );
    ...
    const soldAmount = btcTransactions
      ...
      .reduce(
        (sum: number, tx: { quantity: number }) => sum + Number(tx.quantity),
        0,
      );
