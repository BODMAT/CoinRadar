...
const normalizedSameSite: 'lax' | 'strict' | 'none' =
    cookieSameSite === 'strict' ? 'strict' : cookieSameSite === 'none' ? 'none' : 'lax';
...
const signRefreshToken = (userId: string): string => {
    if (!JWT_REFRESH_SECRET) {
        throw new Error('JWT_REFRESH_SECRET/JWT_SECRET is not defined in environment variables.');
    }
...
const createSession = async (user: UserWithWallets, req: Request, res: Response): Promise<void> => {
...
exports.refreshSession = async (req: Request, res: Response) => {
    try {
        ...
        const nextAccessToken = signAccessToken(existingToken.user.id, existingToken.user.login);
...
exports.getCurrentUser = async (req: Request, res: Response) => {
    try {
        ...
        return res.status(500).json({ error: 'Server error during current user fetch.' });
    }
};
