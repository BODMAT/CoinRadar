...
exports.createSwap = async (req: Request, res: Response) => {
  try {
    ...
    if (fromCoin === toCoin) {
      return res
        .status(400)
        .json({ error: "fromCoin and toCoin must be different." });
    }
    ...
    for (
      let attempt = 1;
      attempt <= MAX_SWAP_TRANSACTION_RETRIES;
      attempt += 1
    ) {
      try {
        const transactionResult = await prisma.$transaction(
          async (tx: any) => {
            ...
            const createdSellTx = await tx.transaction.create({
              data: {
                walletId,
                coinSymbol: fromCoin,
                buyOrSell: "sell",
            ...
            const createdBuyTx = await tx.transaction.create({
              data: {
                walletId,
                coinSymbol: toCoin,
                buyOrSell: "buy",
    ...
    if (!sellTx || !buyTx) {
      return res
        .status(500)
        .json({ error: "Swap transaction failed unexpectedly." });
    }
