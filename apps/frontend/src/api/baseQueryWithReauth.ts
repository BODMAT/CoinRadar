...
import { clearWalletState, setWalletsList } from "../modules/Wallet/selectedWallet.slice";
...
> = async (args, api, extraOptions) => {
    ...
    if (result.error?.status === 401 && !isAuthBypassRequest(args)) {
        ...
        if (refreshResult.data && typeof refreshResult.data === "object" && refreshResult.data !== null && "user" in refreshResult.data) {
            try {
                const parsedUser: UserSafe = UserSchema.parse((refreshResult.data as { user: unknown }).user);
