...
export function DeleteWalletPopup() {
  ...
  const selectedWallet = useAppSelector((state) =>
    state.selectedWallet.walletsList.find(
      (wallet) => wallet.id === selectedWalletId,
    ),
  );

  const [deleteWallet, { isLoading, isError, error, isSuccess }] =
  ...
  return (
    ...
    >
      <p className="text-lg text-[--color-text] text-center">
        Are you absolutely sure that you want to remove the wallet{" "}
        {selectedWallet.name}? This action cannot be canceled.
      </p>
