  ...
  const handleAddWallet = () => {
    if (!user) {
      dispatch(
        openPopup({
          title: "Failure",
          children: "You need to be logged in to add a wallet",
        }),
      );
      return;
    } else {
      dispatch(
        openPopup({ title: "Add wallet", children: <AddWalletPopup /> }),
      );
      return;
    }
  };
  const handleDeleteCurrentWallet = () => {
    if (!user) {
      dispatch(
        openPopup({
          title: "Failure",
          children: "You need to be logged in to delete a wallet",
        }),
      );
      return;
    } else {
      if (!selectedWallet) {
        dispatch(
          openPopup({
            title: "Delete wallet",
            children: "You need to select a wallet to delete",
          }),
        );
      } else {
        dispatch(
  ...
  const handleUpdateCurrentWallet = () => {
    if (!user) {
      dispatch(
        openPopup({
          title: "Failure",
          children: "You need to be logged in to update a wallet",
        }),
      );
      return;
    } else {
      if (!selectedWallet) {
        dispatch(
          openPopup({
            title: "Update wallet",
            children: "You need to select a wallet to update",
          }),
        );
      } else {
        dispatch(
  ...
  return (
    <div className="flex justify-between max-md:flex-col items-center gap-7 bg-(image:--color-background) rounded-2xl p-3">
      <div className="fontTitle text-2xl">
        {!user && (
          <h1 className="fontTitle text-2xl">
      ...
      <div className="flex gap-5 max-[520px]:flex-col">
        ...
        <button
          disabled={!user}
          onClick={handleUpdateCurrentWallet}
          className="max-[500px]:w-full flex justify-center items-center text-center px-9 py-2 bg-(--color-card) cursor-pointer rounded transitioned hover:scale-105 text-[--color-text] border-[--color-text] border-2 disabled:cursor-not-allowed"
        >
        ...
        <button
          disabled={!user}
          onClick={handleDeleteCurrentWallet}
          className="max-[500px]:w-full flex justify-center items-center text-center px-9 py-2 bg-(--color-card) cursor-pointer rounded transitioned hover:scale-105 text-[--color-text] border-[--color-text] border-2 disabled:cursor-not-allowed"
        >
