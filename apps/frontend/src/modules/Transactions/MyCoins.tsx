...
export function MyCoins() {
  ...
  const { data: userCoins } = useGetAllTransactionsGroupByCoinSymbolQuery(
    selectedWalletId || "",
  ...
  const userCoinWithAPIData = useMemo(() => {
    ...
    return userCoins.map((userCoin) => {
      ...
      const pnlValue =
        (currentPrice - userCoin.avgBuyingPrice) * userCoin.totalQuantity;
  ...
  const handleOpenCoinPopup = (symbol: string) => () => {
    dispatch(
      openPopup({
        title: "Coin info",
        children: <WatchTransactionsPopup coinSymbol={symbol} />,
      }),
    );
  };

  if (!selectedWalletId)
    return <div className="text-center p-4">Select a wallet</div>;
  if (!userCoins || !allCoins)
  ...
  return (
    <div className="p-4">
      <div className="w-full flex justify-between gap-10 mb-5 items-center max-[350px]:flex-col">
        <h1 className="fontTitle text-2xl font-bold text-(--color-text)">
          My coins:
        </h1>
        <div className="w-48">
          <Select
            ...
            >
              <div className="flex items-center gap-3">
                <img
                  ...
                  onError={(e) => {
                    e.currentTarget.src =
                      "https://placehold.co/32x32/1f2937/fff?text=?";
                  }}
                />
                <span className="font-bold uppercase max-[400px]:hidden">
                  {coin.coinSymbol}
                </span>
              </div>
              <div className="text-right font-mono">
                {formatQuantity(coin.totalQuantity)}
              </div>
              <div className="text-right font-mono text-green-400">
                {formatPrice(coin.currentPrice!)}
              </div>
              <div className="text-right font-mono max-[460px]:hidden text-gray-400">
                ${coin.avgBuyingPrice.toFixed(4)}
              </div>
              <div
                className={`text-right font-mono font-bold ${
                  (coin.PNL || 0) >= 0 ? "text-green-500" : "text-red-500"
                }`}
              >
                {(coin.PNL || 0) >= 0 ? "+" : ""}
                {formatQuantity(coin.PNL || 0)}$
              </div>
            </button>
          ))
        )}
