...
export function Header() {
  ...
  const { data: wallet } = useGetWalletQuery(selectedWalletId || "", {
  ...
  const stats = useMemo(() => {
    ...
    const currentBalance = portfolio.reduce((acc, item) => {
      const apiCoin = allCoins.find(
        (c) => c.symbol.toLowerCase() === item.coinSymbol.toLowerCase(),
      );
  ...
  const handleOpenChooseCoinPopup = () => {
    if (!user) {
      dispatch(
        openPopup({
          title: "Failure",
          children: "You need to be logged in to add a transaction",
        }),
      );
      return;
    }
    dispatch(
      openPopup({ title: "Choose coin", children: <ChooseCoinPopup /> }),
    );
  };
  ...
  const handleOpenWatchTransactionsPopup = () => {
    if (!user) {
      dispatch(
        openPopup({
          title: "Failure",
          children: "You need to be logged in to watch transactions",
        }),
      );
      return;
    }
    dispatch(
      openPopup({
        title: "Transactions",
        children: <WatchTransactionsPopup />,
      }),
    );
  };
  ...
  const handleOpenSwapSettingsPopup = () => {
    if (!user) {
      dispatch(
        openPopup({
          title: "Failure",
          children: "You need to be logged in to change swap settings",
        }),
      );
      return;
    }
    dispatch(
      openPopup({ title: "Swap Settings", children: <SwapSettingsPopup /> }),
    );
  };
    ...
    <div className="flex justify-between max-lg:justify-evenly max-md:flex-col items-center gap-7 bg-(image:--color-background) rounded-2xl p-3">
      <div className="flex gap-7 items-center max-[420px]:flex-col">
        ...
        <div className="flex flex-col">
          <div className="flex gap-3 items-center max-[1000px]:flex-col">
            ...
            <h2
              className={`font-mono text-xl font-bold ${stats.pnl >= 0 ? "text-green-500" : "text-red-500"}`}
            >
              {stats.pnl >= 0 ? "+" : ""}
          ...
          <div className="flex gap-3 items-center max-[1000px]:flex-col">
            ...
            <h2
              className={`font-mono text-xl font-bold ${stats.realizedPnL >= 0 ? "text-green-500" : "text-red-500"}`}
            >
              {stats.realizedPnL >= 0 ? "+" : ""}
          ...
          <div className="flex gap-3 items-center max-[1000px]:flex-col">
            ...
            <h2
              className={`font-mono text-xl font-bold ${stats.totalPnL >= 0 ? "text-green-500" : "text-red-500"}`}
            >
              {stats.totalPnL >= 0 ? "+" : ""}
      ...
      <div className="flex gap-5 max-[920px]:flex-col">
        <button
          ...
          onClick={handleOpenChooseCoinPopup}
          className="max-[500px]:w-full flex justify-center items-center text-center px-9 py-2 bg-(--color-card) cursor-pointer rounded transitioned hover:scale-105 text-[--color-text] border-[--color-text] border-2 disabled:cursor-not-allowed"
        >
          Add transaction
        </button>
        <button
          disabled={!selectedWalletId}
          onClick={handleOpenWatchTransactionsPopup}
          className="max-[500px]:w-full flex justify-center items-center text-center px-9 py-2 bg-(--color-card) cursor-pointer rounded transitioned hover:scale-105 text-[--color-text] border-[--color-text] border-2 disabled:cursor-not-allowed"
        >
          View transactions
        </button>
        <button
          disabled={!selectedWalletId}
          onClick={handleOpenSwapSettingsPopup}
          className="max-[500px]:w-full flex justify-center items-center text-center px-4 py-2 bg-(--color-card) cursor-pointer rounded transitioned hover:scale-105 text-[--color-text] border-[--color-text] border-2 disabled:cursor-not-allowed"
        >
          Swap settings
        </button>
      </div>
