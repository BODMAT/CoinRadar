...
export function ChooseCoinPopup() {
  ...
  const { data: allCoins, isLoading, isError } = useGetAllCoinsQuery();
    ...
    <div>
      ...
      <div className="flex flex-col gap-3">
        {filteredCoins.map((coin: Coin) => (
          <button
            key={coin.id}
            onClick={() => handleOpenAddTransaction(coin)}
            className="grid grid-cols-3 gap-2 justify-between items-center transitioned hover:scale-101 hover:shadow-md hover:bg-(--color-card) p-2 cursor-pointer hover:text-(--color-text)"
          >
