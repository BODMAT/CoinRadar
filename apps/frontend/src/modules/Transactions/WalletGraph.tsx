...
export function WalletGraph() {
  ...
  const { data: agregatedTransactions } =
    useGetGroupedTransactionsForChartQuery(
  ...
  const chartData: Point[] = useMemo(() => {
    if (
      !agregatedTransactions ||
      !allCoins ||
      agregatedTransactions.length === 0
    ) {
      return [];
    }
    ...
    const timeScale: number[] = Array.from(
      { length: NUM_POINTS },
      (_, i) => startTime + i * MS_PER_HOUR,
    );

    const currentNetQuantity: Record<string, number> = {};
    const txPointers: Record<string, number> = {};
    const marketDataMap: Record<string, (typeof allCoins)[0]> = allCoins.reduce(
      (acc, coin) => {
    ...
    timeScale.forEach((time, index) => {
      ...
      agregatedTransactions.forEach((coinGroup) => {
        ...
        while (
          ...
          currentNetQuantity[symbol] +=
            tx.buyOrSell === "buy" ? tx.quantity : -tx.quantity;
          txPointers[symbol]++;
        }
    ...
    agregatedTransactions.forEach((coinGroup) => {
      ...
      const currentBalance = userCoins?.find(
        (coin) => coin.coinSymbol === symbol,
      )?.totalQuantity;
  ...
  const isGrowing =
    chartData.length > 1 && chartData.at(-1)!.y > chartData[0]!.y;
  const lineColor = isGrowing ? "#4caf50" : "#f44336";
  const fillColor = isGrowing
    ? "rgba(76, 175, 80, 0.2)"
    : "rgba(244, 67, 54, 0.2)";
