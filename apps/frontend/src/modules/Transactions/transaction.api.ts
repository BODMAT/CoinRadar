...
import {
  ...
  TransactionResponseArraySchema,
} from "./transaction.schema";
import type { CoinForChart, CoinInfo } from "./coinInfo.schema";
import {
  CoinForChartArraySchema,
  CoinInfoArraySchema,
  CoinInfoSchema,
} from "./coinInfo.schema";
...
export const transactionApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    createTransaction: builder.mutation<
      Transaction,
      { walletId: string; data: CreateTransaction }
    >({
      query: ({ walletId, data }) => ({
      ...
      onQueryStarted: async (_, { dispatch, queryFulfilled }) => {
        try {
          ...
          console.error(
            "Failed to invalidate wallet tags after createTransaction",
            error,
          );
        }
      },
    ...
    >({
      ...
      onQueryStarted: async (_, { dispatch, queryFulfilled }) => {
        try {
          ...
          console.error(
            "Failed to invalidate wallet tags after deleteTransaction",
            error,
          );
        }
      },
    ...
    >({
      ...
      onQueryStarted: async (_, { dispatch, queryFulfilled }) => {
        try {
          ...
          console.error(
            "Failed to invalidate wallet tags after updateTransaction",
            error,
          );
        }
      },
    ...
    getTransactionsByCoin: builder.query<
      Transaction[],
      { walletId: string; coinSymbol: string }
    >({
      query: ({ walletId, coinSymbol }) =>
        `wallets/${walletId}/transactions/coins/${coinSymbol}`,
      providesTags: (result) =>
        result
          ? [
              ...result.map(({ id }) => ({ type: "Transaction" as const, id })),
    ...
    >({
      query: ({ walletId, coinSymbol }) =>
        `wallets/${walletId}/transactions/coins/${coinSymbol}/stats`,
      providesTags: [{ type: "Transaction", id: "LIST" }],
      transformResponse: (response: unknown) => {
    ...
    getGroupedTransactionsForChart: builder.query<
      CoinForChart[],
      { walletId: string; range?: string }
    >({
