...
export function ChangeTransactionPopup({
  transactionId,
}: {
  ...
  const coinApiData = allCoins?.find(
  ...
  const handleSubmit = async () => {
    const availableToSell =
      currentCoinInWallet +
      (transaction.buyOrSell === "sell" ? transaction.quantity : 0);
    ...
    try {
      ...
      setTimeout(() => {
        dispatch(
          openPopup({ title: "Success", children: "Transaction updated!" }),
        );
      }, 300);
    } catch (error: unknown) {
      setAlert(extractApiErrorMessage(error, "Failed to update transaction"));
    }
  };
    ...
    <div className="flex flex-col gap-5 p-2">
      <div className="flex justify-between gap-5 items-center mx-auto max-md:flex-col border-b pb-4 w-full">
        ...
        <div className="text-right text-sm">
          ...
          <p>
            Avg Buy: <b>${coinStats?.avgBuyingPrice}</b>
          </p>
        </div>
      </div>
