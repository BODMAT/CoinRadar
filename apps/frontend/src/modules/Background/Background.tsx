...
export function Background() {
  ...
  useEffect(() => {
    const ids = Array.from(document.querySelectorAll('[id^="section"]')).map(
      (el) => el.id,
    );
    setSectionIds(ids);
  }, []);
  ...
  useEffect(() => {
    ...
    timeoutRef.current = setTimeout(() => {
      ...
      if (!closestSection || closestSection === currentSectionIdRef.current)
        return;
  ...
  useEffect(() => {
    ...
    const setupCanvas = () => {
      ...
      const count = Math.max(
        45,
        Math.min(120, Math.floor((width * height) / 18000)),
      );
