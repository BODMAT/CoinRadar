...
export function AllCrypto() {
  ...
  const { page, perPage, searchQuery } = useAppSelector(
    (state) => state.allCrypto,
  );
  ...
  const handleOpenPopup = (coin: Coin) => {
    ...
    dispatch(
      openPopup({
        title: `${coin.name} about`,
        children: <CoinPopup coin={coin} />,
      }),
    );
  };
      ...
      {(isInitialLoading || hasCoins) && !isNotFound && (
        <div className="mt-3 bg-(image:--color-background) rounded-2xl py-2">
          <div className="grid grid-cols-7 max-md:grid-cols-6 max-[600px]:grid-cols-5! max-[450px]:grid-cols-4! max-[370px]:grid-cols-3! gap-5 px-4 my-3 items-center">
            <h3 className="text-xl font-black text-left max-[450px]:hidden">
              #
            </h3>
            <h3 className="text-xl font-black text-left">Name</h3>
            <h3 className="text-xl font-black text-left max-[600px]:hidden">
              Image
            </h3>
            <h3 className="text-xl font-black text-left">Price</h3>
            <h3 className="text-xl font-black text-left max-[370px]:hidden">
              24h
            </h3>
            <h3 className="text-xl font-black text-left col-span-2 max-md:col-span-1">
              Last 7 days
            </h3>
          </div>
            ...
            {coinsToDisplay.map((coin: Coin | undefined, index: number) => {
              ...
              return (
                ...
                >
                  ...
                  <h3
                    className={`max-[370px]:hidden ${!isSkeleton ? (coin.price_change_percentage_24h > 0 ? "text-green-500" : "text-red-500") : ""}`}
                  >
                    {isSkeleton ? (
