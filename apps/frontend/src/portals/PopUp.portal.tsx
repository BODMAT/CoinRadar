...
export function PopUp({ withoutSizes = false }: { withoutSizes?: boolean }) {
      ...
      {active && (
        ...
        >
          <motion.div
            ...
            className={
              "fontText bg-(--color-card) text-(--color-text) backdrop-blur-md rounded-lg w-[90%] max-w-[700px] shadow-lg relative z-999999! max-md:mt-[5vh] max-h-[80vh] flex flex-col overflow-hidden " +
              (withoutSizes ? "" : "")
            }
          >
            <div className="bg-[#121212] flex justify-between items-center p-4 shrink-0">
              <h2 className="fontTitle text-2xl font-bold text-white">
                {title}
              </h2>
              <button
